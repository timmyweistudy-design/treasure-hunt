...
### 2026-05-20（v4）視覺 & 內容優化

**傳送門視覺重設計**
- 移除 🌀 emoji，改為純 CSS 三層結構：外環順時針旋轉 + 內環虛線逆時針旋轉 + 中心脈衝圓
- 配對同色：tl↔br = 青藍 `#00e5ff`（`.portal-pair-cyan`）、tr↔bl = 琥珀 `#ff9800`（`.portal-pair-amber`）
- 觸發半徑圓顏色同步跟隨配對色

**建築碰撞顏色加深**
- `fillOpacity` `.09` → `.28`，`opacity` `.45` → `.70`，顏色換成深磚紅 `#d84315`
- 建築範圍在地圖上更明顯易辨識

**首頁說明更新**
- `templates/index.html` 遊戲說明從 4 點擴充為 6 點，涵蓋：10 個寶藏、公尺數計分、3 種敵人、5 格道具欄、手雷/地雷/誘餌/傳送門、天氣/連擊

**規則頁面數值全面校正**
- 誘餌 8s→6s、冷卻 40s；連擊窗口 30s→22s；手雷補充爆炸半徑/暈眩/充能；地雷補充觸發半徑/暈眩/自動消除；通緝說明修正；傳送門 2→4 個；黃金寶藏補充限時；追跡者補充生成時機；小偷補充隱身；新增時間壓力與衝刺說明
